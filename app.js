...
// POST Routes
fastify.post('/api/checkusername/', (request, reply) => {
  const { entryId } = request.body;

  connection.query('SELECT * FROM userbase WHERE username = ?', [entryId], (error, results) => {
    if (error) {
      reply.send({ error: 'An error occurred while checking the entry.' });
